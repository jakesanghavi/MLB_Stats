...
def plot_legend(ax_breaks_legend, df_game, unique_pitch_types, pitch_colors_dict, desc_to_code_dict):
    ...
    ax_breaks_legend.legend(
        ...
        handlelength=2.5,
        fontsize=9,
        borderpad=0.1,  # reduce padding around legend box
        labelspacing=0.2,  # reduce vertical space between labels
        handletextpad=0.5,  # reduce space between handle and label text
...
def plot_pitcher_dashboard(player, date, player_info, pbp_df, pitch_map, year=date.today().year):
    ...
    #     hspace=0.05  # Add this parameter to control vertical spacing between rows
    # )
    ...
    outer_gs = GridSpec(5, 1, height_ratios=[1, 1, 0.01, 4, 1])

    # --- Top Block: Title, Image ---
    top_gs = GridSpecFromSubplotSpec(1, 5, subplot_spec=outer_gs[0], height_ratios=[0.7], hspace=-0)
    ax_title = fig.add_subplot(top_gs[0, :3])
    ax_img = fig.add_subplot(top_gs[0, 3:4])

    top2_gs = GridSpecFromSubplotSpec(1, 5, subplot_spec=outer_gs[1], height_ratios=[0.7], hspace=0)
    ax_table = fig.add_subplot(top2_gs[0, :])

    tm_gs = GridSpecFromSubplotSpec(1, 5, subplot_spec=outer_gs[2], height_ratios=[0.5], hspace=0)
    ax_breaks_legend = fig.add_subplot(tm_gs[0, :])


    mid_gs = GridSpecFromSubplotSpec(2, 5, subplot_spec=outer_gs[3], height_ratios=[0.5, 2], hspace=0)
    ax_rhb = fig.add_subplot(mid_gs[1, :2])
    ax_mirror_bar = fig.add_subplot(mid_gs[1, 2:3])
    ax_lhb = fig.add_subplot(mid_gs[1, 3:])

    # --- Bottom Block: Breaks, Velo, Stats ---
    bot_gs = GridSpecFromSubplotSpec(2, 5, subplot_spec=outer_gs[4], height_ratios=[2, 1], hspace=0)
    ax_breaks = fig.add_subplot(bot_gs[0, 1:3])
    ax_velocities = fig.add_subplot(bot_gs[0, 3:])
    ax_stats = fig.add_subplot(bot_gs[1, :])
