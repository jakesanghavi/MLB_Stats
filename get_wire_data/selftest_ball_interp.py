...
def test_missing_apex_beats_linear():
    """Parabola y = 4t - t^2 (peak 4 at t=2). Drop the top; cubic should
    overshoot the chord toward the true peak, using neighbors on both sides.
    """
    ts = [i * 0.25 for i in range(0, 17)]  # 0 .. 4
    full = [(t, t, 4 * t - t * t, 0.0) for t in ts]
    # drop samples with y > 3 (around the apex)
    track = [p for p in full if p[2] <= 3.0 + 1e-9]
    assert len(track) < len(full)
    t_peak = 2.0
    got = sample_ball(track, t_peak)
    assert got is not None
    # bracketing samples sit on y=3; linear midpoint would be 3
    ...
    # stay below a wild overshoot of the true peak
    assert got[1] < 4 * t_peak - t_peak * t_peak + 1.5
